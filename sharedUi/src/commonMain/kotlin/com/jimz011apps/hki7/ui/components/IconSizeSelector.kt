    ...
    Column(modifier.fillMaxWidth()) {
        Row(
            ...
            Column(Modifier.weight(1f)) {
                Text(
                    stringResource(Res.string.stack_child_icon_only),
                    style = MaterialTheme.typography.labelLarge
                )
                Text(
                    stringResource(Res.string.stack_child_icon_only_description),
        ...
        if (childIconOnly) {
            IconSizeSelector(
                ...
                label = stringResource(Res.string.stack_child_icon_size)
            )
        }
    }
}
...
fun IconSizeSelector(
    ...
    modifier: Modifier = Modifier,
    label: String = stringResource(Res.string.button_icon_size)
) {
    Column(modifier.fillMaxWidth().padding(top = 8.dp)) {
        Text(label, style = MaterialTheme.typography.labelLarge)
        Text(
            stringResource(Res.string.button_icon_size_description),
            style = MaterialTheme.typography.bodySmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant
        )
        FlowRow(
            ...
            FilterChip(
                ...
                label = { Text(stringResource(Res.string.button_icon_size_auto)) }
            )
            ICON_SIZE_OPTIONS.forEach { size ->
                FilterChip(
                    selected = value == size,
                    onClick = { onValueChange(size) },
                    label = { Text(stringResource(Res.string.button_icon_size_value, size)) }
                )
            }
        }
    }
